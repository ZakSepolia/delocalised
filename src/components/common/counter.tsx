...
const Counter: React.FC<CounterProps> = ({
  ...
  return (
    <div
      className={cn(
        "group flex items-center justify-between rounded-md overflow-hidden flex-shrink-0 bg-[#F9AFD2] ",
        {
          "border h-11 md:h-12 border-gray-300": variant === "default",
          "h-8 md:h-9 shadow-navigation text-black bg-white border border-gray-300   ":
            variant === "dark",
        }
      )}
    >
      <button
        ...
        className={cn(
          ...
          {
            "w-10 md:w-12 text-black border-e border-pink-200 hover:text-black ":
              variant === "default",
            "w-8 md:w-9 text-black bg-white  focus:outline-none border border-gray-100":
              variant === "dark",
          }
        )}
      ...
      <span
        className={cn(
          ...
          {
            "text-base text-black w-12  md:w-20 xl:w-24": variant === "default",
            "text-sm text-black w-8 md:w-10  ": variant === "dark",
          }
        )}
      >
        {quantity}
      ...
      <button
        ...
        className={cn(
          ...
          {
            "w-10 md:w-12 text-black border-s border-pink-200 hover:text-black ":
              variant === "default",
            "w-8 md:w-9 text-black bg-white   focus:outline-none border border-gray-100":
              variant === "dark",
          }
